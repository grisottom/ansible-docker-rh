...
#previously downloaded 'ojdbc8.jar' driver
DRIVER_JAR=$TMP_DIR_DOWNLOADS/downloads/oracle/driver/ojdbc8.jar
if [ ! -f $DRIVER_JAR ]; then
  echo_message "Oracle Driver not found in $DRIVER_JAR"
  exit 1
fi

#module to be created
TMP_MODULE_DIR=$TMP_DIR_DOWNLOADS/jboss/module-oracle
...
MODULE_ORACLE_DIR=$TMP_MODULE_DIR/modules/com/oracle/ojdbc8/main

#create pg module folder
mkdir -p $MODULE_ORACLE_DIR

#copy postgres module 'module.xml' configuration to $MODULE_PG_DIR
cp module.xml $MODULE_ORACLE_DIR/

#copy previously downloaded 'postgresql.jar' driver to $MODULE_PG_DIR
cp $DRIVER_JAR $MODULE_ORACLE_DIR/
